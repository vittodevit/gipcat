...
$res = $con->query("SELECT `version` FROM `interventions_nb` WHERE `idIntervention` = '$id'");
...
$fieldnames = array("interventionState", "assignedTo",
"interventionDate", "interventionDuration", "footNote");
                    
$fields = "";

foreach ($fieldnames as $fn){
    // super ugly bugfix, ill fix it when i can
    if($fn == "interventionState"){
        if (isset($_POST[$fn])) {
